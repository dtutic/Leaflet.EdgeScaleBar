...
L.EdgeScaleBar = L.Layer.extend({
	...
	_create_lat_ticks: function() {
		...
		for	(i = d_s+this._dd/2; i	< d_g; i = i + this._dd) {
			...
			if ((phi < this._LIMIT_PHI) && (phi > -this._LIMIT_PHI)) this._draw_lat_tick(phi,10);
		}
		...
		for	(i = d_s; i	< d_g; i = i + this._dd/10.0) {
			var phi = this._invmerLength(i);			
			if ((phi < this._LIMIT_PHI) && (phi > -this._LIMIT_PHI)) this._draw_lat_tick(phi,4);
		}
	...
	_create_lon_ticks: function() {
		...
		for	(i = cen_p.lng+dl/2; i < right_p.lng; i = i + dl) {
			this._draw_lon_tick(i,10);
		}
		for	(i = cen_p.lng-dl/2; i > left_p.lng; i = i - dl) {
			this._draw_lon_tick(i,10);
		}
		for	(i = cen_p.lng; i < right_p.lng; i = i + dl/10) {
			this._draw_lon_tick(i,4);
		}
		for	(i = cen_p.lng-dl/10; i > left_p.lng; i = i - dl/10) {
			this._draw_lon_tick(i,4);
		}
	},
	...
	_draw_lat_tick: function(phi,size) {
	...
	_draw_lon_tick: function(lam,size) {
